...
+ (JUMPJID *)jidWithUser:(NSString *)user domain:(NSString *)domain resource:(NSString *)resource {
    if ([JUMPJID validateUser:user domain:domain resource:resource]) {
        JUMPJID *jid = [[JUMPJID alloc] init];
        jid->user = [user copy];
        jid->domain = [domain copy];
        jid->resource = [resource copy];
...
+ (BOOL)parse:(NSString *)jidStr outUser:(NSString **)user outDomain:(NSString **)domain outResource:(NSString **)resource {
    ...
    if (atRange.location != NSNotFound) {
        ...
        jidStr = [jidStr substringFromIndex:atRange.location + 1];
        // check /
        NSRange slashRange = [jidStr rangeOfString:@"/"];
        if (slashRange.location != NSNotFound) {
            // parse domain & resource
    ...
    // validate
    if ([JUMPJID validateUser:rawUser domain:rawUser resource:rawResource]) {
        if (user) {
            *user = rawUser;
        }
        if (domain) {
...
+ (BOOL)validateDomain:(NSString *)domain {
    // domain is the only required part of a JUMPJID
    if (domain == nil || [domain length] == 0) {
        return NO;
    }
    
    return [self validatePart:domain];
}
...
+ (BOOL)validateResource:(NSString *)resource {
    ...
    return [self validatePart:resource];
}

+ (BOOL)validatePart:(NSString *)part {
    NSString *regex = @"^[A-Za-z0-9_\\-\\.]{1,1024}$";
