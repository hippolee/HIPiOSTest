...
- (void)commonInit {
    ...
    //双击本来是自动对焦的，现在更换成放大缩小摄像头
//    _doubleTapToResetFocusGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapToContinouslyAutoFocus:)];
    _doubleTapToResetFocusGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapToChangeZoom:)];
...
- (void)ChangeZoom {
    ...
    if (_zoomAtStart == 1) {
        strongRecorder.videoZoomFactor = 2;
    } else {
        strongRecorder.videoZoomFactor = 1;
    }
}
...
/**
 *  本来双击事件是用来自动对焦的，目前不使用了，双击事件留给摄像头缩放
 *
...
- (void)tapToChangeZoom:(UIGestureRecognizer *)gestureRecognizer {
    SCRecorder *strongRecorder = self.recorder;
    _zoomAtStart = strongRecorder.videoZoomFactor;
