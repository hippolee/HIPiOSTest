...
- (void)exportAsynchronouslyWithCompletionHandler:(SCAssetExportSessionExportCompleteBlock)completionHandler {
    _cancelled = NO;
    ...
    if (![_reader startReading]) {
        if (_reader.error != nil) {
            _error = _reader.error;
            if (completionHandler != nil) {
                completionHandler(NO, _reader.error);
            }
            return;
        }
    }
    ...
    if (![_writer startWriting]) {
        if (_writer.error != nil) {
            _error = _writer.error;
            if (completionHandler != nil) {
                completionHandler(NO, _writer.error);
            }
            return;
        }
    }
