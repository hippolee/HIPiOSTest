...
NSString *const JUMPSRVResolverErrorDomain = @"JUMPSRVResolverErrorDomain";

// Log levels: off, error, warn, info, verbose
#if DEBUG
static const int jumpLogLevel = JUMP_LOG_LEVEL_WARN; // | JUMP_LOG_FLAG_TRACE;
#else
...
@interface JUMPSRVRecord ()

@property(nonatomic, assign) NSUInteger srvResultsIndex;
@property(nonatomic, assign) NSUInteger sum;

- (NSComparisonResult)compareByPriority:(JUMPSRVRecord *)aRecord;

@end

#pragma mark -

@implementation JUMPSRVResolver
...
- (id)initWithdDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq resolverQueue:(dispatch_queue_t)rq {
    ...
    if ((self = [super init])) {
        ...
        if (rq) {
            ...
            resolverQueue = dispatch_queue_create("JUMPSRVResolver", NULL);
        }
...
- (void)sortResults {
    ...
    while ([results count] > 0) {
        ...
        if (srvResultsCount == 1) {
            JUMPSRVRecord *srvRecord = [results objectAtIndex:0];
...
- (void)succeed {
    ...
    dispatch_async(delegateQueue, ^{ @autoreleasepool {
        ...
        SEL selector = @selector(jumpSRVResolver:didResolveRecords:);
        
        if ([theDelegate respondsToSelector:selector]) {
            [theDelegate jumpSRVResolver:self didResolveRecords:records];
        } else {
            JUMPLogWarn(@"%@: delegate doesn't implement %@", THIS_FILE, NSStringFromSelector(selector));
        }
...
- (void)failWithError:(NSError *)error {
    ...
    JUMPLogTrace2(@"%@: %@ %@", THIS_FILE, THIS_METHOD, error);
    ...
    if (delegateQueue != NULL) {
        dispatch_async(delegateQueue, ^{ @autoreleasepool {
            ...
            SEL selector = @selector(jumpSRVResolver:didNotResolveDueToError:);
            
            if ([theDelegate respondsToSelector:selector]) {
                [theDelegate jumpSRVResolver:self didNotResolveDueToError:error];
            } else {
                JUMPLogWarn(@"%@: delegate doesn't implement %@", THIS_FILE, NSStringFromSelector(selector));
            }
...
- (void)failWithDNSError:(DNSServiceErrorType)sdErr {
    JUMPLogTrace2(@"%@: %@ %i", THIS_FILE, THIS_METHOD, (int)sdErr);
    
    [self failWithError:[NSError errorWithDomain:JUMPSRVResolverErrorDomain code:sdErr userInfo:nil]];
}

- (JUMPSRVRecord *)processRecord:(const void *)rdata length:(uint16_t)rdlen {
    JUMPLogTrace();
    ...
    if (rdata == NULL) {
        JUMPLogWarn(@"%@: %@ - rdata == NULL", THIS_FILE, THIS_METHOD);
        return nil;
    }
    ...
    if (rr != NULL) {
        ...
        if (target != nil) {
            ...
            result = [JUMPSRVRecord recordWithPriority:priority weight:weight port:port target:target];
        }
...
static void QueryRecordCallback(DNSServiceRef       sdRef,
    ...
    JUMPSRVResolver *resolver = (__bridge JUMPSRVResolver *)context;
    ...
    if (errorCode == kDNSServiceErr_NoError && rrtype == kDNSServiceType_SRV) {
        JUMPSRVRecord *record = [resolver processRecord:rdata length:rdlen];
        if (record) {
...
- (void)startWithSRVName:(NSString *)aSRVName timeout:(NSTimeInterval)aTimeout {
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        JUMPLogTrace2(@"%@: startWithSRVName:%@ timeout:%f", THIS_FILE, aSRVName, aTimeout);
        ...
        dispatch_source_set_event_handler(sdReadSource, ^{ @autoreleasepool {
            ...
            JUMPLogVerbose(@"%@: sdReadSource_eventHandler", THIS_FILE);
        ...
        dispatch_source_set_cancel_handler(sdReadSource, ^{ @autoreleasepool {
            ...
            JUMPLogVerbose(@"%@: sdReadSource_cancelHandler", THIS_FILE);
        ...
        if (timeout > 0.0) {
            ...
            dispatch_source_set_event_handler(timeoutTimer, ^{ @autoreleasepool {
                ...
                NSError *err = [NSError errorWithDomain:JUMPSRVResolverErrorDomain code:0 userInfo:userInfo];
...
+ (NSString *)srvNameFromJUMPDomain:(NSString *)jumpDomain {
    if (jumpDomain == nil) {
        ...
        return [NSString stringWithFormat:@"_jump-client._tcp.%@", jumpDomain];
    }
}
...
+ (JUMPSRVRecord *)recordWithPriority:(UInt16)p1 weight:(UInt16)w port:(UInt16)p2 target:(NSString *)t {
    return [[JUMPSRVRecord alloc] initWithPriority:p1 weight:w port:p2 target:t];
}
