...
- (id)initWithDispatchQueue:(dispatch_queue_t)queue {
    if ((self = [super initWithDispatchQueue:queue])) {
        ...
        reconnectDelay = DEFAULT_JUMP_RECONNECT_DELAY;
        reconnectTimerInterval = DEFAULT_JUMP_RECONNECT_TIMER_INTERVAL;
...
- (void)manualStart {
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        if ([jumpStream isDisconnected] && [self manuallyStarted] == NO) {
...
- (void)jumpStream:(JUMPStream *)sender didReceiveError:(JUMPError *)error {
    ...
    // <stream:error>
    //   <conflict xmlns="urn:ietf:params:xml:ns:jump-streams"/>
    //   <text xmlns="urn:ietf:params:xml:ns:jump-streams" xml:lang="">Replaced by new connection</text>
    // </stream:error>
    //
...
- (void)jumpStreamDidDisconnect:(JUMPStream *)sender withError:(NSError *)error {
    ...
    if ([self autoReconnect] && [self shouldReconnect]) {
        ...
        [multicastDelegate jumpReconnect:self didDetectAccidentalDisconnect:reachabilityFlags];
    }
    
    if ([self multipleReachabilityChanges]) {
        // While the previous connection attempt was in progress, the reachability of the jump host changed.
...
static void JUMPReconnectReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info) {
    @autoreleasepool {
        
        JUMPReconnect *instance = (__bridge JUMPReconnect *)info;
...
- (void)setupNetworkMonitoring {
    ...
    if (reachability == NULL) {
        NSString *domain = jumpStream.hostName;
        if (domain == nil) {
            domain = @"im.yyuap.com";
        }
        ...
        if (reachability) {
            ...
            SCNetworkReachabilitySetCallback(reachability, JUMPReconnectReachabilityCallback, &context);
            
            if (moduleQueue) {
                SCNetworkReachabilitySetDispatchQueue(reachability,moduleQueue);
            } else {
                JUMPLogWarn(@"%@: %@ - No jump moduleQueue!", THIS_FILE, THIS_METHOD);
            }
        }
    }
}
...
- (void)maybeAttemptReconnectWithReachabilityFlags:(SCNetworkReachabilityFlags)reachabilityFlags {
    ...
    if (([self manuallyStarted]) || ([self autoReconnect] && [self shouldReconnect])) {
        if ([jumpStream isDisconnected] && ([self queryingDelegates] == NO)) {
            // The jump stream is disconnected, and is not attempting reconnection
            ...
            SEL selector = @selector(jumpReconnect:shouldAttemptAutoReconnect:);
            ...
            while ([delegateEnumerator getNextDelegate:&del delegateQueue:&dq forSelector:selector]) {
                dispatch_group_async(delGroup, dq, ^{ @autoreleasepool {
                    ...
                    if (![del jumpReconnect:self shouldAttemptAutoReconnect:reachabilityFlags]) {
            ...
            dispatch_async(concurrentQueue, ^{ @autoreleasepool {
                ...
                dispatch_async(moduleQueue, ^{ @autoreleasepool {
                    ...
                    if (shouldAttemptReconnect) {
                        ...
                        [jumpStream connectWithTimeout:JUMPStreamTimeoutNone error:nil];
            ...
            // The jump stream is already attempting a connection.
            
            if (reachabilityFlags != previousReachabilityFlags) {
                // It seems that the reachability of our jump host has changed in the middle of either
