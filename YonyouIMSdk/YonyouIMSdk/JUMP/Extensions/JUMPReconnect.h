...
#define _JUMP_RECONNECT_H

#define DEFAULT_JUMP_RECONNECT_DELAY 2.0

#define DEFAULT_JUMP_RECONNECT_TIMER_INTERVAL 20.0

@protocol JUMPReconnectDelegate;

/**
 * JUMPReconnect handles automatically reconnecting to the jump server due to accidental disconnections.
 * That is, a disconnection that is not the result of calling disconnect on the jump stream.
 *
 ...
 * These issue have been largely overcome due to the keepalive implementation in JUMPStream.
 *
 * Regardless of how the disconnect happens, the JUMPReconnect class can help to automatically re-establish
 * the jump stream so as to have minimum impact on the user (and hopefully they don't even notice).
 *
 ...
 * When the reachability of the jump host has changed, a reconnect may be tried again.
 ...
 * first launches, and the jump stream is unable to connect to the host.
 * In cases such as this it may be desireable to start monitoring the network for reachability changes.
 * This way when internet connectivity is restored, one can immediately connect the jump stream.
 ...
 **/
@interface JUMPReconnect : JUMPModule {
 ...
 *
 * The default value is DEFAULT_JUMP_RECONNECT_DELAY (defined at the top of this file).
 *
 ...
 *
 * The default value is DEFAULT_JUMP_RECONNECT_TIMER_INTERVAL (defined at the top of this file).
 *
 * To disable this feature, set the value to zero.
 *
 * Note: NSTimeInterval is a double that specifies the time in seconds.
 ...
 * This may be useful, for example, if one needs network monitoring in order to setup the inital jump connection.
 ...
 * and the class will begin monitoring the network for changes in reachability to the jump host.
 *
 * A manual start of the reconnect process will effectively end once the jump stream has been opened.
 * That is, if you invoke manualStart and the jump stream is later opened,
 * then future disconnections will not result in an auto reconnect process (unless the autoReconnect property applies).
 *
 * This method does nothing if the jump stream is not disconnected.
 **/
- (void)manualStart;
 ...
 * That is, if the autoReconnect property is still enabled, and the jump stream is later opened, authenticated and
