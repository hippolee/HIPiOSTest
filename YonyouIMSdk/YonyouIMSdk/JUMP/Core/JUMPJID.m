...
#import "JUMPJID.h"
#import "JUMPStringPrep.h"

@implementation JUMPJID
...
+ (BOOL)parse:(NSString *)jidStr outUser:(NSString **)user outDomain:(NSString **)domain outResource:(NSString **)resource {
    ...
    if (atRange.location != NSNotFound) {
        ...
        if (slashRange.location != NSNotFound) {
            rawUser = [jidStr substringToIndex:slashRange.location];
    ...
    NSString *prepUser = [JUMPStringPrep prepNode:rawUser];
    NSString *prepDomain = [JUMPStringPrep prepDomain:rawDomain];
    NSString *prepResource = [JUMPStringPrep prepResource:rawResource];
    
    if ([JUMPJID validateUser:prepUser domain:prepDomain resource:prepResource]) {
...
+ (JUMPJID *)jidWithString:(NSString *)jidStr {
    ...
    if ([JUMPJID parse:jidStr outUser:&user outDomain:&domain outResource:&resource]) {
        JUMPJID *jid = [[JUMPJID alloc] init];
        jid->user = [user copy];
...
+ (JUMPJID *)jidWithString:(NSString *)jidStr resource:(NSString *)resource {
    NSString *prepResource = [JUMPStringPrep prepResource:resource];
    if (![self validateResource:prepResource]) {
        return nil;
    }
    ...
    if ([JUMPJID parse:jidStr outUser:&user outDomain:&domain outResource:nil]) {
        JUMPJID *jid = [[JUMPJID alloc] init];
        jid->user = [user copy];
        jid->domain = [domain copy];
...
+ (JUMPJID *)jidWithUser:(NSString *)user domain:(NSString *)domain resource:(NSString *)resource {
    NSString *prepUser = [JUMPStringPrep prepNode:user];
    NSString *prepDomain = [JUMPStringPrep prepDomain:domain];
    NSString *prepResource = [JUMPStringPrep prepResource:resource];
...
+ (JUMPJID *)jidWithPrevalidatedUser:(NSString *)user prevalidatedDomain:(NSString *)domain prevalidatedResource:(NSString *)resource {
    JUMPJID *jid = [[JUMPJID alloc] init];
    jid->user = [user copy];
    jid->domain = [domain copy];
    jid->resource = [resource copy];
...
+ (JUMPJID *)jidWithPrevalidatedUser:(NSString *)user prevalidatedDomain:(NSString *)domain resource:(NSString *)resource {
    NSString *prepResource = [JUMPStringPrep prepResource:resource];
    if (![self validateResource:prepResource]) {
        return nil;
    }
...
- (JUMPJID *)bareJID {
    if (resource == nil) {
        ...
        return [JUMPJID jidWithPrevalidatedUser:user prevalidatedDomain:domain prevalidatedResource:nil];
    }
}
...
- (JUMPJID *)domainJID {
    if (user == nil && resource == nil) {
        ...
        return [JUMPJID jidWithPrevalidatedUser:nil prevalidatedDomain:domain prevalidatedResource:nil];
    }
}
...
- (JUMPJID *)jidWithNewResource:(NSString *)newResource {
    return [JUMPJID jidWithPrevalidatedUser:user prevalidatedDomain:domain resource:newResource];
}
...
- (BOOL)isEqual:(id)anObject {
    if ([anObject isMemberOfClass:[self class]]) {
        return [self isEqualToJID:(JUMPJID *)anObject options:JUMPJIDCompareFull];
    }
    return NO;
}

- (BOOL)isEqualToJID:(JUMPJID *)aJID {
    return [self isEqualToJID:aJID options:JUMPJIDCompareFull];
}
...
- (BOOL)isEqualToJID:(JUMPJID *)aJID options:(JUMPJIDCompareOptions)mask {
    ...
    if (mask & JUMPJIDCompareResource) {
        if (resource) {
