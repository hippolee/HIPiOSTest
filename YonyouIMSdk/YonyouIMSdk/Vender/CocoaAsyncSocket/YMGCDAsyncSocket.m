...
#ifndef YMGCDAsyncSocketLoggingEnabled
#define YMGCDAsyncSocketLoggingEnabled 0
#endif

#if YMGCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext YMGCDAsyncSocketLoggingContext
...
#ifndef YMGCDAsyncSocketLogLevel
#define YMGCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = YMGCDAsyncSocketLogLevel;
...
NSString *const YMGCDAsyncSocketException = @"YMGCDAsyncSocketException";
NSString *const YMGCDAsyncSocketErrorDomain = @"YMGCDAsyncSocketErrorDomain";

NSString *const YMGCDAsyncSocketQueueName = @"YMGCDAsyncSocket";
NSString *const YMGCDAsyncSocketThreadName = @"YMGCDAsyncSocket-CFStream";

NSString *const YMGCDAsyncSocketManuallyEvaluateTrust = @"YMGCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const YMGCDAsyncSocketUseCFStreamForTLS = @"YMGCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const YMGCDAsyncSocketSSLPeerID = @"YMGCDAsyncSocketSSLPeerID";
NSString *const YMGCDAsyncSocketSSLProtocolVersionMin = @"YMGCDAsyncSocketSSLProtocolVersionMin";
NSString *const YMGCDAsyncSocketSSLProtocolVersionMax = @"YMGCDAsyncSocketSSLProtocolVersionMax";
NSString *const YMGCDAsyncSocketSSLSessionOptionFalseStart = @"YMGCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const YMGCDAsyncSocketSSLCipherSuites = @"YMGCDAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const YMGCDAsyncSocketSSLDiffieHellmanParameters = @"YMGCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum YMGCDAsyncSocketFlags
{
...
@interface YMGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation YMGCDAsyncSocketPreBuffer
...
/**
 * The YMGCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface YMGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(YMGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation YMGCDAsyncReadPacket
...
/**
 * The YMGCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface YMGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation YMGCDAsyncWritePacket
...
/**
 * The YMGCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface YMGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation YMGCDAsyncSpecialPacket
...
@implementation YMGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	...
	YMGCDAsyncReadPacket *currentRead;
	YMGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	YMGCDAsyncSocketPreBuffer *preBuffer;
	...
	YMGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([YMGCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[YMGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak YMGCDAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong YMGCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create YMGCDAsyncSocket instance for accepted socket
			
			YMGCDAsyncSocket *acceptedSocket = [[YMGCDAsyncSocket alloc] initWithDelegate:theDelegate
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			NSMutableArray *addresses = [YMGCDAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
			...
			else
			{
				...
				{
					if (!address4 && [YMGCDAsyncSocket isIPv4Address:address])
					{
						address4 = address;
					}
					else if (!address6 && [YMGCDAsyncSocket isIPv6Address:address])
					{
						address6 = address;
					}
				}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YMGCDAsyncSocketConnectTimeoutError",
	                                                     @"YMGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YMGCDAsyncSocketReadMaxedOutError",
														 @"YMGCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YMGCDAsyncSocketReadTimeoutError",
	                                                     @"YMGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YMGCDAsyncSocketWriteTimeoutError",
	                                                     @"YMGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YMGCDAsyncSocketClosedError",
	                                                     @"YMGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YMGCDAsyncSocketErrorDomain code:YMGCDAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the YMGCDAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	YMGCDAsyncReadPacket *packet = [[YMGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[YMGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[YMGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued YMGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued YMGCDAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via YMGCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		YMGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		YMGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	YMGCDAsyncWritePacket *packet = [[YMGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[YMGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[YMGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued YMGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued YMGCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a YMGCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		YMGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	YMGCDAsyncSpecialPacket *packet = [[YMGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			YMGCDAsyncSpecialPacket *tlsPacket = (YMGCDAsyncSpecialPacket *)currentRead;
			NSDictionary *tlsSettings = tlsPacket->tlsSettings;
			
			NSNumber *value = [tlsSettings objectForKey:YMGCDAsyncSocketUseCFStreamForTLS];
...
{
	YMGCDAsyncSocket *asyncSocket = (__bridge YMGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:YMGCDAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. YMGCDAsyncSocketSSLPeerID
	//  4. YMGCDAsyncSocketSSLProtocolVersionMin
	//  5. YMGCDAsyncSocketSSLProtocolVersionMax
	//  6. YMGCDAsyncSocketSSLSessionOptionFalseStart
	//  7. YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. YMGCDAsyncSocketSSLCipherSuites
	//  9. YMGCDAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. YMGCDAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. YMGCDAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. YMGCDAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. YMGCDAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. YMGCDAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:YMGCDAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YMGCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for YMGCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use YMGCDAsyncSocketSSLProtocolVersionMin & YMGCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[YMGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"YMGCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[YMGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[YMGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
	...
	// So this mostly affects newer protocols (JUMP, IMAP) as opposed to older protocols (HTTPS).
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("YMGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)stopCFStreamThreadIfNeeded
{
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
		...
		if (--cfstreamThreadRetainCount == 0)
		{
			...
			// wake up the thread
			[YMGCDAsyncSocket performSelector:@selector(ignore:)
...
{
	[[NSThread currentThread] setName:YMGCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(YMGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(YMGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	YMGCDAsyncSocket *asyncSocket = (__bridge YMGCDAsyncSocket *)pInfo;
