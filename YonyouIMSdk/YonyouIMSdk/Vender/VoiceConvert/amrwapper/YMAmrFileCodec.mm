...
void SkipToPCMAudioData(FILE* fpwave)
{
    YM_RIFFHEADER riff;
    YM_FMTBLOCK fmt;
    YM_XCHUNKHEADER chunk;
    YM_WAVEFORMATX wfx;
    int bDataBlock = 0;
    
    // 1. 读RIFF头
    fread(&riff, 1, sizeof(YM_RIFFHEADER), fpwave);
    
    // 2. 读FMT块 - 如果 fmt.nFmtSize>16 说明需要还有一个附属大小没有读
    fread(&chunk, 1, sizeof(YM_XCHUNKHEADER), fpwave);
    if ( chunk.nChunkSize>16 )
    {
        fread(&wfx, 1, sizeof(YM_WAVEFORMATX), fpwave);
    }
    else
    {
        memcpy(fmt.chFmtID, chunk.chChunkID, 4);
        fmt.nFmtSize = chunk.nChunkSize;
        fread(&fmt.wf, 1, sizeof(YM_WAVEFORMAT), fpwave);
    }
...
long ReadPCMFrame(short speech[], FILE* fpwave, int nChannels, int nBitsPerSample)
{
    ...
    // 原始PCM音频帧数据
    unsigned char  pcmFrame_8b1[YM_PCM_FRAME_SIZE];
    unsigned char  pcmFrame_8b2[YM_PCM_FRAME_SIZE<<1];
    unsigned short pcmFrame_16b1[YM_PCM_FRAME_SIZE];
    unsigned short pcmFrame_16b2[YM_PCM_FRAME_SIZE<<1];
    
    if (nBitsPerSample==8 && nChannels==1)
    {
        nRead = fread(pcmFrame_8b1, (nBitsPerSample/8), YM_PCM_FRAME_SIZE*nChannels, fpwave);
        for(x=0; x<YM_PCM_FRAME_SIZE; x++)
        {
        ...
        {
            nRead = fread(pcmFrame_8b2, (nBitsPerSample/8), YM_PCM_FRAME_SIZE*nChannels, fpwave);
            for( x=0, y=0; y<YM_PCM_FRAME_SIZE; y++,x+=2 )
            {
                // 1 - 取两个声道之左声道
            ...
            {
                nRead = fread(pcmFrame_16b1, (nBitsPerSample/8), YM_PCM_FRAME_SIZE*nChannels, fpwave);
                for(x=0; x<YM_PCM_FRAME_SIZE; x++)
                {
                ...
                {
                    nRead = fread(pcmFrame_16b2, (nBitsPerSample/8), YM_PCM_FRAME_SIZE*nChannels, fpwave);
                    for( x=0, y=0; y<YM_PCM_FRAME_SIZE; y++,x+=2 )
                    {
    ...
    if (nRead<YM_PCM_FRAME_SIZE*nChannels) {
        return 0;
    }
    return nRead;
}
...
int YM_EncodeWAVEFileToAMRFile(const char* pchWAVEFilename, const char* pchAMRFileName, int nChannels, int nBitsPerSample)
{
    FILE* fpwave;
    FILE* fpamr;
    ...
    /* bitstream filetype */
    unsigned char amrFrame[YM_MAX_AMR_FRAME_SIZE];
    ...
    bytes = fwrite(YM_AMR_MAGIC_NUMBER, sizeof(char), strlen(YM_AMR_MAGIC_NUMBER), fpamr);
...
void WriteWAVEFileHeader(FILE* fpwave, int nFrame)
{
    ...
    + sizeof(YM_XCHUNKHEADER)               // fmt
    + sizeof(YM_WAVEFORMATX)           // WAVEFORMATX
    + sizeof(YM_XCHUNKHEADER)               // DATA
    + nFrame*160*sizeof(short);    //
    strcpy(tag, "WAVE");
    memcpy(riff.chRiffFormat, tag, 4);
    fwrite(&riff, 1, sizeof(YM_RIFFHEADER), fpwave);
    ...
    chunk.nChunkSize = sizeof(YM_WAVEFORMATX);
    fwrite(&chunk, 1, sizeof(YM_XCHUNKHEADER), fpwave);
    memset(&wfx, 0, sizeof(YM_WAVEFORMATX));
    wfx.nFormatTag = 1;
    wfx.nChannels = 1; // 单声道
    ...
    fwrite(&wfx, 1, sizeof(YM_WAVEFORMATX), fpwave);
...
int caclAMRFrameSize(unsigned char frameHeader)
{
    ...
    temp2 = myround((double)(((double)mode / (double)YM_AMR_FRAME_COUNT_PER_SECOND) / (double)8));
...
// 将AMR文件解码成WAVE文件
int YM_DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
    FILE* fpamr = NULL;
    FILE* fpwave = NULL;
    char magic[8];
    ...
    unsigned char amrFrame[YM_MAX_AMR_FRAME_SIZE];
    short pcmFrame[YM_PCM_FRAME_SIZE];
    ...
    // 检查amr文件头
    fread(magic, sizeof(char), strlen(YM_AMR_MAGIC_NUMBER), fpamr);
    if (strncmp(magic, YM_AMR_MAGIC_NUMBER, strlen(YM_AMR_MAGIC_NUMBER)))
    {
        fclose(fpamr);
        return 0;
    }
    ...
    nFrameCount++;
    fwrite(pcmFrame, sizeof(short), YM_PCM_FRAME_SIZE, fpwave);
