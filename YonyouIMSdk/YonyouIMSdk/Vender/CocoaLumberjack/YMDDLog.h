 ...
 *
 * Step 3:
 * Replace your NSLog statements with YMDDLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> YMDDLogError(@"Fatal error, no dohickey found!");
 * 
 * YMDDLog works exactly the same as NSLog.
...
@class YMDDLogMessage;

@protocol YMDDLogger;
@protocol YMDDLogFormatter;
 ...
 * 
 * The YMDDLogMessage object includes a 'tag' ivar that may be used for a variety of purposes.
 ...
 * as the YMDDLogMessage class captures the individual flag that caused the log message to fire.
 * 
 * -- Levels --
 * 
 ...
 *         YMDDLogVerbose(@"sprocket: %@", [sprocket description])
 * }
 * 
 * -- Async --
 * 
...
#define YMDDLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   LOG_LEVEL_DEF, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define YMDDLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    LOG_LEVEL_DEF, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define YMDDLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    LOG_LEVEL_DEF, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define YMDDLogDebug(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_DEBUG,   LOG_LEVEL_DEF, LOG_FLAG_DEBUG,   0, frmt, ##__VA_ARGS__)
#define YMDDLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, LOG_LEVEL_DEF, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

#define YMDDLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   LOG_LEVEL_DEF, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define YMDDLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    LOG_LEVEL_DEF, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define YMDDLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    LOG_LEVEL_DEF, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define YMDDLogCDebug(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_DEBUG,   LOG_LEVEL_DEF, LOG_FLAG_DEBUG,   0, frmt, ##__VA_ARGS__)
#define YMDDLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, LOG_LEVEL_DEF, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

/**
 * The THIS_FILE macro gives you an NSString of the file name.
 * For simplicity and clarity, the file name does not include the full path or file extension.
 * 
 * For example: YMDDLogWarn(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
**/

NSString *YMDDExtractFileNameWithoutExtension(const char *filePath, BOOL copy);

#define THIS_FILE (YMDDExtractFileNameWithoutExtension(__FILE__, NO))

/**
 * The THIS_METHOD macro gives you the name of the current objective-c method.
 * 
 * For example: YMDDLogWarn(@"%@ - Requires non-nil strings", THIS_METHOD) -> @"setMake:model: requires non-nil strings"
 * 
...
+ (void)addLogger:(id <YMDDLogger>)logger;    // adds the logger using maximum log level (LOG_LEVEL_VERBOSE)

/**
 * Please use as logLevels the LOG_LEVEL_* macros
 *
**/
+ (void)addLogger:(id <YMDDLogger>)logger withLogLevel:(int)logLevel;

+ (void)removeLogger:(id <YMDDLogger>)logger;
...
**/
- (id <YMDDLogFormatter>)logFormatter;
- (void)setLogFormatter:(id <YMDDLogFormatter>)formatter;
 ...
 * 
 * This allows invocations of YMDDLog's flushLog method to be propogated to loggers that need it.
 * 
 * Note that YMDDLog's flushLog method is invoked automatically when the application quits,
...
@protocol YMDDLogFormatter <NSObject>
@required
...
@protocol YMDDRegisteredDynamicLogging
...
/**
 * The YMDDLogMessage class encapsulates information about the log message.
 * If you write custom loggers or formatters, you will be dealing with objects of this class.
**/

enum {
    YMDDLogMessageCopyFile     = 1 << 0,
    YMDDLogMessageCopyFunction = 1 << 1
};
typedef int YMDDLogMessageOptions;

@interface YMDDLogMessage : NSObject <NSCopying>
{
...
@public
    ...
    // For 3rd party extensions that manually create YMDDLogMessage instances.
    YMDDLogMessageOptions options;
}
 ...
 * Options is a bitmask which supports YMDDLogMessageCopyFile and YMDDLogMessageCopyFunction.
**/
- (instancetype)initWithLogMsg:(NSString *)logMsg
                      ...
                      function:(const char *)function
                          line:(int)line
                           tag:(id)tag
                       options:(YMDDLogMessageOptions)optionsMask;
...
/**
 * The YMDDLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface YMDDAbstractLogger : NSObject <YMDDLogger>
{
    id <YMDDLogFormatter> formatter;
