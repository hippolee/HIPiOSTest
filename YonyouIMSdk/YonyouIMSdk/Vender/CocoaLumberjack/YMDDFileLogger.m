...
@interface YMDDLogFileManagerDefault (PrivateAPI)
...
@interface YMDDFileLogger (PrivateAPI)
...
@implementation YMDDLogFileManagerDefault
...
- (instancetype)initWithLogsDirectory:(NSString *)aLogsDirectory
{
    if ((self = [super init]))
    {
        ...
        NSLogVerbose(@"YMDDFileLogManagerDefault: logsDirectory:\n%@", [self logsDirectory]);
        NSLogVerbose(@"YMDDFileLogManagerDefault: sortedLogFileNames:\n%@", [self sortedLogFileNames]);
    }
    return self;
}
                       ...
                       context:(void *)context
{
    ...
    {
        NSLogInfo(@"YMDDFileLogManagerDefault: Responding to configuration change: maximumNumberOfLogFiles");
        
        dispatch_async([YMDDLog loggingQueue], ^{ @autoreleasepool {
...
- (void)deleteOldLogFiles
{
    NSLogVerbose(@"YMDDLogFileManagerDefault: deleteOldLogFiles");
    ...
    if (count > 0)
    {
        YMDDLogFileInfo *logFileInfo = [sortedLogFileInfos objectAtIndex:0];
    ...
    {
        YMDDLogFileInfo *logFileInfo = [sortedArchivedLogFileInfos objectAtIndex:i];
        
        NSLogInfo(@"YMDDLogFileManagerDefault: Deleting file: %@", logFileInfo.fileName);
...
- (NSString *)logsDirectory
{
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:_logsDirectory])
    {
        ...
        {
            NSLogError(@"YMDDFileLogManagerDefault: Error creating logsDirectory: %@", err);
        }
    }
...
/**
 * Returns an array of YMDDLogFileInfo objects,
...
- (NSArray *)unsortedLogFileInfos
{
    ...
    for (NSString *filePath in unsortedLogFilePaths)
    {
        YMDDLogFileInfo *logFileInfo = [[YMDDLogFileInfo alloc] initWithFilePath:filePath];
...
- (NSArray *)sortedLogFilePaths
{
    ...
    for (YMDDLogFileInfo *logFileInfo in sortedLogFileInfos)
    {
...
- (NSString *)createNewLogFile
{
    ...
    do
    {
        ...
        {
            NSLogVerbose(@"YMDDLogFileManagerDefault: Creating new log file: %@", actualFileName);
...
@implementation YMDDLogFileFormatterDefault
...
@implementation YMDDFileLogger

- (id)init
{
    YMDDLogFileManagerDefault *defaultLogFileManager = [[YMDDLogFileManagerDefault alloc] init];
    
    return [self initWithLogFileManager:defaultLogFileManager];
}

- (instancetype)initWithLogFileManager:(id <YMDDLogFileManager>)aLogFileManager
{
    if ((self = [super init]))
    {
        ...
        formatter = [[YMDDLogFileFormatterDefault alloc] init];
    }
    return self;
}
...
- (void)scheduleTimerToRollLogFileDueToAge
{
    ...
    NSLogVerbose(@"YMDDFileLogger: scheduleTimerToRollLogFileDueToAge");
    
    NSLogVerbose(@"YMDDFileLogger: logFileCreationDate: %@", logFileCreationDate);
    NSLogVerbose(@"YMDDFileLogger: logFileRollingDate : %@", logFileRollingDate);
...
- (void)rollLogFileNow
{
    NSLogVerbose(@"YMDDFileLogger: rollLogFileNow");
...
- (void)maybeRollLogFileDueToAge
{
    ...
    {
        NSLogVerbose(@"YMDDFileLogger: Rolling log file due to age...");
...
- (void)maybeRollLogFileDueToSize
{
    ...
    if (maximumFileSize > 0)
    {
        ...
        {
            NSLogVerbose(@"YMDDFileLogger: Rolling log file due to size (%qu)...", fileSize);
...
**/
- (YMDDLogFileInfo *)currentLogFileInfo
{
    if (currentLogFileInfo == nil)
    {
        ...
        {
            YMDDLogFileInfo *mostRecentLogFileInfo = [sortedLogFileInfos objectAtIndex:0];
            ...
            {
                NSLogVerbose(@"YMDDFileLogger: Resuming logging with file %@", mostRecentLogFileInfo.fileName);
        ...
        if (currentLogFileInfo == nil)
        {
            ...
            currentLogFileInfo = [[YMDDLogFileInfo alloc] initWithFilePath:currentLogFilePath];
        }
    }
    ...
    if (currentLogFileHandle == nil)
    {
        ...
        if (currentLogFileHandle)
        {
            ...
            dispatch_source_set_event_handler(currentLogFileVnode, ^{ @autoreleasepool {
                NSLogInfo(@"YMDDFileLogger: Current logfile was moved. Rolling it and creating a new one");
                [self rollLogFileNow];
            }});
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark YMDDLogger Protocol
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (logMsg)
    {
        ...
        @catch (NSException *exception) {
            ...
            if (exception_count <= 10) {
                NSLogError(@"YMDDFileLogger.logMessage: %@", exception);
                if (exception_count == 10)
                    NSLogError(@"YMDDFileLogger.logMessage: Too many exceptions -- will not log any more of them.");
            }
        }
    }
}
...
@implementation YMDDLogFileInfo
...
- (void)renameFile:(NSString *)newFileName
{
    ...
    if (![newFileName isEqualToString:[self fileName]])
    {
        ...
        NSLogVerbose(@"YMDDLogFileInfo: Renaming file: '%@' -> '%@'", self.fileName, newFileName);

        NSError *error = nil;
        if (![[NSFileManager defaultManager] fileExistsAtPath:newFilePath]){
            if([[NSFileManager defaultManager] removeItemAtPath:newFilePath error:&error]){
                NSLogError(@"YMDDLogFileInfo: Error deleting archive (%@): %@", self.fileName, error);
            }
        }

        if (![[NSFileManager defaultManager] moveItemAtPath:filePath toPath:newFilePath error:&error])
        {
            NSLogError(@"YMDDLogFileInfo: Error renaming file (%@): %@", self.fileName, error);
        }
...
- (void)addExtendedAttributeWithName:(NSString *)attrName
{
    ...
    if (result < 0)
    {
        NSLogError(@"YMDDLogFileInfo: setxattr(%@, %@): error = %s",
                   attrName,
                   self.fileName,
                   strerror(errno));
    }
...
- (void)removeExtendedAttributeWithName:(NSString *)attrName
{
    ...
    {
        NSLogError(@"YMDDLogFileInfo: removexattr(%@, %@): error = %s",
                   attrName,
                   self.fileName,
                   strerror(errno));
    }
...
- (BOOL)isEqual:(id)object
{
    ...
    {
        YMDDLogFileInfo *another = (YMDDLogFileInfo *)object;
...
- (NSComparisonResult)reverseCompareByCreationDate:(YMDDLogFileInfo *)another
{
...
- (NSComparisonResult)reverseCompareByModificationDate:(YMDDLogFileInfo *)another
{
