...
#define YMDD_DEBUG NO

#define NSLogDebug(frmt, ...) do{ if(YMDD_DEBUG) NSLog((frmt), ##__VA_ARGS__); } while(0)
...
// The "global logging queue" refers to [YMDDLog loggingQueue].
...
@interface YMDDLoggerNode : NSObject {
@public 
    id <YMDDLogger> logger;   
...
+ (YMDDLoggerNode *)nodeWithLogger:(id <YMDDLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue logLevel:(int)logLevel;

@end


@interface YMDDLog (PrivateAPI)

+ (void)lt_addLogger:(id <YMDDLogger>)logger logLevel:(int)logLevel;
+ (void)lt_removeLogger:(id <YMDDLogger>)logger;
+ (void)lt_removeAllLoggers;
+ (NSArray *)lt_allLoggers;
+ (void)lt_log:(YMDDLogMessage *)logMessage;
+ (void)lt_flush;
...
+ (void)initialize
{
    ...
    if (!initialized)
    {
        ...
        NSLogDebug(@"YMDDLog: Using grand central dispatch");
        ...
        NSLogDebug(@"YMDDLog: numProcessors = %u", numProcessors);
...
+ (void)addLogger:(id <YMDDLogger>)logger
{
...
+ (void)queueLogMessage:(YMDDLogMessage *)logMessage asynchronously:(BOOL)asyncFlag
{
     ...
     format:(NSString *)format, ...
{
    ...
    if (format)
    {
        ...
        YMDDLogMessage *logMessage = [[YMDDLogMessage alloc] initWithLogMsg:logMsg
                                                                  level:level
                                                                   flag:flag
                                                                context:context
                                                                   file:file
...
+ (BOOL)isRegisteredClass:(Class)class
{
    ...
    // 
    // The method +[YMDDLog isRegisteredClass] will crash a project when using it with ARC + Simulator.
...
+ (NSArray *)registeredClasses
{
    ...
    // We can now loop through the classes, and test each one to see if it is a YMDDLogging class.
...
+ (void)lt_addLogger:(id <YMDDLogger>)logger logLevel:(int)logLevel
{
    ...
    YMDDLoggerNode *loggerNode = [YMDDLoggerNode nodeWithLogger:logger loggerQueue:loggerQueue logLevel:logLevel];
...
+ (void)lt_removeLogger:(id <YMDDLogger>)logger
{
    ...
    YMDDLoggerNode *loggerNode = nil;
    
    for (YMDDLoggerNode *node in loggers)
    {
    ...
    if (loggerNode == nil)
    {
        NSLogDebug(@"YMDDLog: Request to remove logger which wasn't added");
        return;
    }
...
+ (void)lt_removeAllLoggers
{
    ...
    for (YMDDLoggerNode *loggerNode in loggers)
    {
...
@implementation YMDDLoggerNode

@synthesize logLevel;

- (instancetype)initWithLogger:(id <YMDDLogger>)aLogger loggerQueue:(dispatch_queue_t)aLoggerQueue logLevel:(int)aLogLevel
{
    if ((self = [super init]))
    {
        logger = aLogger;
...
+ (YMDDLoggerNode *)nodeWithLogger:(id <YMDDLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue logLevel:(int)logLevel
{
    return [[YMDDLoggerNode alloc] initWithLogger:logger loggerQueue:loggerQueue logLevel:logLevel];
}
                       ...
                       options:(YMDDLogMessageOptions)optionsMask
{
    if ((self = [super init]))
    {
        ...
        if (options & YMDDLogMessageCopyFile)
            file = dd_str_copy(aFile);
        else
            file = (char *)aFile;
        
        if (options & YMDDLogMessageCopyFunction)
...
- (NSString *)fileName
{
    return YMDDExtractFileNameWithoutExtension(file, NO);
}
...
- (void)dealloc
{
    if (file && (options & YMDDLogMessageCopyFile))
        free(file);
    
    if (function && (options & YMDDLogMessageCopyFunction))
        free(function);
...
- (id)copyWithZone:(NSZone *)zone {
    YMDDLogMessage *newMessage = [[YMDDLogMessage alloc] init];
    ...
    if (self->options & YMDDLogMessageCopyFile) {
...
@implementation YMDDAbstractLogger
...
- (id <YMDDLogFormatter>)logFormatter
{
    ...
    //
    // YMDDLogVerbose(@"log msg 1");
    // YMDDLogVerbose(@"log msg 2");
    // [logger setFormatter:myFormatter];
    // YMDDLogVerbose(@"log msg 3");
    //
    ...
    // IMPORTANT NOTE:
    // 
    // Methods within the YMDDLogger implementation MUST access the formatter ivar directly.
    ...
    __block id <YMDDLogFormatter> result;
...
- (void)setLogFormatter:(id <YMDDLogFormatter>)logFormatter
{
